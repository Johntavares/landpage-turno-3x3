...
export const AuthModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="w-full max-w-md rounded-3xl bg-white p-7 shadow-2xl dark:bg-slate-900 border border-slate-200 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Acesse sua conta ou crie um perfil para sincronizar sua escala
          </p>
        </div>

        {/* BOTÕES DE REDES SOCIAIS (GOOGLE & APPLE) */}
        <div className="space-y-2.5 mb-5">
          <button
            type="button"
            onClick={() => loginWithProvider('google')}
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 py-2.5 px-4 text-xs font-bold text-slate-700 dark:text-slate-200 shadow-xs hover:bg-slate-50 dark:hover:bg-slate-700/80 transition-all active:scale-[0.98]"
          >
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 rounded-2xl border border-slate-900 dark:border-slate-700 bg-slate-900 dark:bg-slate-800 py-2.5 px-4 text-xs font-bold text-white shadow-xs hover:bg-slate-800 dark:hover:bg-slate-700 transition-all active:scale-[0.98]"
          >
            {/* Ícone vetorizado da Apple */}
        ...
        {/* TABS DE ALTERNÂNCIA (ENTRAR / CRIAR CONTA) */}
        <div className="grid grid-cols-2 gap-1.5 p-1 rounded-2xl bg-slate-100 dark:bg-slate-800/80 mb-5">
          <button
            type="button"
            onClick={() => {
              setMode('login');
        ...
        {authError && (
          <div className="mb-4 p-3 rounded-2xl bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-300 text-xs flex items-center gap-2">
        ...
        {resetSent && (
          <div className="mb-4 p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 py-2.5 pl-10 pr-10 text-sm font-medium text-slate-900 dark:text-white focus:border-blue-500 focus:outline-hidden"
                />
                <button
                  type="button"
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-linear-to-r from-blue-600 to-indigo-600 py-3.5 font-bold text-white shadow-lg shadow-blue-500/30 hover:from-blue-700 hover:to-indigo-700 transition-all mt-3 active:scale-[0.98] disabled:opacity-50"
          >
            {isLoading ? (
              <span className="text-xs">Aguarde...</span>
            ) : mode === 'login' ? (
              <>
                <LogIn className="h-4 w-4" /> Entrar no Turno 3x3
              </>
            ) : mode === 'register' ? (
              <>
        ...
        {/* OPÇÃO DE MODO CONVIDADO / TESTE OFFLINE */}
